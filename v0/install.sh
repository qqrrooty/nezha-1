...
NZ_AGENT_PATH="${NZ_BASE_PATH}/agent-1"
...
update_script() {
    ...
    mv -f /tmp/nezha.sh ./nezha-1.sh && chmod a+x ./nezha-1.sh
...
install_arch() {
    ...
    case $input in
    [yY][eE][sS] | [yY])
        useradd -m nezha-agent-1
        sed -i "$ a\nezha-agent-1 ALL=(ALL ) NOPASSWD:ALL" /etc/sudoers
        sudo -iu nezha-agent-1 bash -c 'gpg --keyserver keys.gnupg.net --recv-keys 4695881C254508D1;
        ...
        sed -i '/nezha-agent-1/d' /etc/sudoers && sleep 30s && killall -u nezha-agent-1 && userdel -r nezha-agent-1
        info "提示: 已删除用户nezha-agent-1，请务必手动核查一遍！"
        ;;
    [nN][oO] | [nN])
        echo "不安装libselinux"
        ;;
...
modify_agent_config() {
    ...
    _cmd="sudo ${NZ_AGENT_PATH}/nezha-agent-1 service install -s $nz_grpc_host:$nz_grpc_port -p $nz_client_secret $args >/dev/null 2>&1"

    if ! eval "$_cmd"; then
        sudo "${NZ_AGENT_PATH}"/nezha-agent-1 service uninstall >/dev/null 2>&1
        sudo "${NZ_AGENT_PATH}"/nezha-agent-1 service install -s "$nz_grpc_host:$nz_grpc_port" -p "$nz_client_secret" "$args" >/dev/null 2>&1
    fi
...
show_agent_log() {
    ...
    if [ "$os_alpine" != 1 ]; then
        sudo journalctl -xf -u nezha-agent-1.service
    else
        sudo tail -n 10 /var/log/nezha-agent-1.err
    fi
...
uninstall_agent() {
    ...
    sudo ${NZ_AGENT_PATH}/nezha-agent-1 service uninstall
...
show_usage() {
    ...
    echo "./nezha-1.sh install_dashboard          - 安装面板端"
    echo "./nezha-1.sh modify_dashboard_config    - 修改面板配置"
    echo "./nezha-1.sh start_dashboard            - 启动面板"
    echo "./nezha-1.sh stop_dashboard             - 停止面板"
    echo "./nezha-1.sh restart_and_update         - 重启并更新面板"
    echo "./nezha-1.sh show_dashboard_log         - 查看面板日志"
    echo "./nezha-1.sh uninstall_dashboard        - 卸载管理面板"
    echo "--------------------------------------------------------"
    echo "./nezha-1.sh install_agent              - 安装监控Agent"
    echo "./nezha-1.sh modify_agent_config        - 修改Agent配置"
    echo "./nezha-1.sh show_agent_log             - 查看Agent日志"
    echo "./nezha-1.sh uninstall_agent            - 卸载Agent"
    echo "./nezha-1.sh restart_agent              - 重启Agent"
    echo "./nezha-1.sh update_script              - 更新脚本"
    echo "--------------------------------------------------------"
}
